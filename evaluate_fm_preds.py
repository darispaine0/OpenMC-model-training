#!/usr/bin/env python3
"""
evaluate_fm_preds.py (fixed)

Reconstruct FMs from predicted ln(coefficients), compute dominant eigenvalue
using the user-supplied power-iteration, and compare to OpenMC keff (if present).
"""
import numpy as np
import torch
...
CHECKPOINT_PATH = RESULTS_DIR / "fm_coefficient_model_best.pth"

INPUT_TEMPS_PATH = Path("training_data/input_temps_final.npy")
FM_NORMALIZED_PATH = Path("training_data/output_fm_normalized_final.npy")
FM_294K_CANDIDATES = [
    Path("training_data/output_fm_normalized294k.npy"),
    Path("training_data/output_fm_normalized_294k.npy"),
    Path("training_data/output_fm_normalized294K.npy"),
    Path("training_data/output_fm_normalized294k.npy"),
    Path("results/consolidated_datasets/train_output_fm_normalized294k.npy"),
    Path("results/consolidated_datasets/output_fm_normalized294k.npy")
]
KEFF_CANDIDATES = [
...
# -------------- Model class (parametrized) --------------
class FMCoefficientPredictor(nn.Module):
    def __init__(self, input_size=578, hidden_size=289, output_size=289*289):
        super().__init__()
...
def main():
    ...
    # squeeze any leading singleton dimensions if necessary
    if fm_294k.ndim > 2:
        fm_294k = np.squeeze(fm_294k)

    # if 1D flattened vector, reshape to square
    if fm_294k.ndim == 1:
    ...
    # load checkpoint (weights_only can be used to avoid unpickling non-weights in newer torch)
    ...
    if cfg and isinstance(cfg, dict):
        hidden_size = int(cfg.get("HIDDEN_SIZE", cfg.get("HIDDEN", N_CELLS)))
    else:
        hidden_size = N_CELLS
    ...
    # reconstruct predicted FMs, run user power iteration
    ...
    np.save(RESULTS_DIR / "predicted_kfm.npy", kfm_arr)
    np.save(RESULTS_DIR / "predicted_kfm_indices.npy", np.array(valid_idx, dtype=int))
    np.save(RESULTS_DIR / "pcm_errors.npy", pcm_arr)
    if openmc_arr.size > 0 and not np.all(np.isnan(openmc_arr)):
        np.save(RESULTS_DIR / "openmc_keff_for_eval.npy", openmc_arr)
    ...
    print(f"\nSaved: {RESULTS_DIR / 'predicted_kfm.npy'} , {RESULTS_DIR / 'pcm_errors.npy'}")
